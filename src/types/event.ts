...
export type ICSEvent = {
   ...
   *
   * Populated during assembly when multiple VEVENT components share the
   * same UID. Each override has a RECURRENCE-ID identifying which instance
   * of the series it replaces.
   *
   * Only present on the base event (the one with RRULE and no RECURRENCE-ID).
   * Override events are removed from the top-level calendar.events array
   * and are only accessible through this field.
   *
   * @see ICSEvent.recurrenceId
   */
  overrides?: ICSEvent[]
}
